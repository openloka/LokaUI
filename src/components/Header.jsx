    ...
    <header
        ...
        {navLinks.map((link) =>
          link.to.startsWith("/") ? (
            <Link
              key={link.label}
              to={link.to}
              className="text-[13px] font-medium text-text-secondary px-3.5 py-1.5 rounded-lg hover:text-text-primary hover:bg-bg-hover transition-colors">
              {link.label}
            </Link>
          ) : (
            <a
      ...
      <div className="flex items-center gap-2.5">
        ...
        <Link
          ...
          className="text-[13px] font-semibold px-4.5 py-2 rounded-[10px] bg-accent text-accent-text hover:bg-accent-hover active:scale-[0.96] transition-all">
          Get started
        </Link>
      </div>
    </header>
